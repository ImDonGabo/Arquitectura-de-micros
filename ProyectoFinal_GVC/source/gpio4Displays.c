...
void vfnDisplayRPM(uint32_t rpm_value) {
    // Asegurarse de que el valor no exceda 4 dígitos (9999)
    if (rpm_value > 9999) {
        rpm_value = 9999; // O mostrar un error como FFFF
    }

    // Extraer cada dígito del valor de RPM
    gbaDisplayData[3] = rpm_value % 10;;         // Unidades
    rpm_value /= 10;
    gbaDisplayData[2] = rpm_value % 10;;         // Decenas
    rpm_value /= 10;
    gbaDisplayData[1] = rpm_value % 10;;         // Centenas
    rpm_value /= 10;
    gbaDisplayData[0] = rpm_value % 10;         // Miles
}
